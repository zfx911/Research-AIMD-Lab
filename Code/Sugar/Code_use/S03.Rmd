...
Condition 1: BlockName=SQ02, BookletPart=1, BlockInPart=1, currentindex=3.

Condition 2: BlockName=SQ02, BookletPart=2, BlockInPart=2, currentindex=17.
...
### Choice in Sequence TA
Filter the data by PSIVariableIdentifier == 11120. Then group by the student id, the new column SQ02S03_choice_sequence should created based on the sequence of the value of the column information in the order of if the row number is higher, their sequence should be higher.
```{r}
...
Then fill in all the missing value of the column SQ02S03_choice_sequence according to the student name.
```{r}
...
### Final Choice TA
Create a new column SQ02S02_final_choice. Filter the data by using PSIVariableIdentifier == 11120 and IsfinalAnswer ==1, then groupby idstud, SQ02S02_final_choice = information.
```{r}
...
### Choice in Sequence TB
Filter the data by PSIVariableIdentifier == 11121. Then group by the student id, the new column SQ02S03_choice_sequence should created based on the sequence of the value of the column information in the order of if the row number is higher, their sequence should be higher.
```{r}
# Filter data and arrange by idstud and information
df_filtered <- result_time_complete_Q3 %>%
...
### Final Choice TB
Create a new column SQ02S02_final_choice. Filter the data by using PSIVariableIdentifier == 11121 and IsfinalAnswer ==1, then groupby idstud, SQ02S02_final_choice = information.
```{r}
...
### Word Count
Filter the data based on PSIVariableIdentifier == 11122 & IsFinalAnswer == 1.
```{r}
# Filter the data based on PSIVariableIdentifier == 11122
filtered_data <- result_time_complete_Q3 %>% filter(PSIVariableIdentifier == 11122)%>%
filter(IsFinalAnswer == 1)
```
...
Create a new column SQ02S03_wordcount_punc that count the number of words in the column information including the punctuations.
```{r}
...
filtered_data$SQ02S03_wordcount_punc <- sapply(filtered_data$information, function(x) length(split_words_and_punctuations(x)))
```
...
```{r}
filtered_data <- mutate(filtered_data, SQ02S03_wordcount_no_punc = str_count(gsub("[-[:punct:]]", " ", information), "\\S+"))

filtered_data[c("idstud","information","module_path","SQ02S03_wordcount_punc","SQ02S03_wordcount_no_punc")]
```

Merge the data to the original data
```{r}
df_filtered_merge = filtered_data[c("idstud","SQ02S03_wordcount_punc","SQ02S03_wordcount_no_punc")]
```

```{r}
# Merge the result back to the original data frame
result_wordcount_Q3 <- merge(result_Final_choice_Q3_TB, df_filtered_merge, by = "idstud", all.x = TRUE)
result_wordcount_Q3
```


### Raw Response
Create a new column SQ02S03_raw_response that is exactly same as the column information of the filtered data.
```{r}
filtered_data$SQ02S03_raw_response <- filtered_data$information

df_filtered_merge = filtered_data[c("idstud","SQ02S03_raw_response")]
```
...
If PSIVariableIdentifier == 11120, then step_str == "TA_select_option_" + value of the column information
If PSIVariableIdentifier == 11121, then step_str == "TB_select_option_" + value of the column information
If PSIVariableIdentifier == 11122, then step_str == "write_explanation"
If eventname == "Nav:GoTo" & new_currentindex == 3, then step_str == "Navigate_to_Page04_Q3"
If eventname == "Nav:GoTo" & new_currentindex != 3, then step_str == if new_currentindex in 1,2,3,5,6,7,8,9,10, then paste0("Navigate_to_Page", new_currentindex+1, "Q",cor_rule(new_currentindex)), else paste0("Navigate_to_Page", new_currentindex+1)
...
result_str_Q3<- result_copy_Q3 %>%
  mutate(
    step_str = case_when(
      PSIVariableIdentifier == 11120 ~ paste0("TA_select_option_", information),
      PSIVariableIdentifier == 11120 ~ paste0("TB_select_option_", information),
      PSIVariableIdentifier == 11122 ~ "write_explanation",
...
df_filtered <- result_str_use_Q3 %>%
  ...
  group_by(idstud) %>%
  mutate(SQ02S03_allsteps_list = list(step_str[order(row_number())]))

# Convert the list column to a new column with the desired format, and then drop the list column
df_filtered$SQ02S03_allsteps_seq <- sapply(df_filtered$SQ02S03_allsteps_list, function(x) paste0("(", toString(x), ")"))
```

```{r}
df_filtered <- select(df_filtered, -SQ02S03_allsteps_list)
df_filtered_merge = df_filtered[c("idstud","RowNumber","SQ02S03_allsteps_seq")]
...
result_allsteps_Q3=result_allsteps_Q3%>%
  group_by(idstud) %>%
  fill(SQ02S03_allsteps_seq, .direction = "downup")

result_allsteps_Q3
```

```{r}
result_allsteps_Q3[c("idstud","SQ02S03_allsteps_seq","information")]
```
...
df_filtered <- result_str_use_Q3 %>%
  group_by(idstud) %>%
  mutate(SQ02S03_allsteps_list = list(SQ02S03_time[order(row_number())]))

# Convert the list column to a new column with the desired format, and then drop the list column
df_filtered$SQ02S03_allsteps_seq_time <- sapply(df_filtered$SQ02S03_allsteps_list, function(x) paste0("(", toString(x), ")"))

# filter the data and select column to merge
df_filtered <- select(df_filtered, -SQ02S03_allsteps_list)
df_filtered_merge = df_filtered[c("idstud","RowNumber","SQ02S03_allsteps_seq_time")]
...
result_alls_time_Q3=result_alls_time_Q3%>%
  group_by(idstud) %>%
  fill(SQ02S03_allsteps_seq_time, .direction = "downup")
result_alls_time_Q3
```
...
df_filtered <- result_str_use_Q3 %>%
  group_by(idstud) %>%
  mutate(SQ02S03_allsteps_list = list(SQ02S03_unit_time[order(row_number())]))

# Convert the list column to a new column with the desired format, and then drop the list column
df_filtered$SQ02S03_allsteps_timestamp <- sapply(df_filtered$SQ02S03_allsteps_list, function(x) paste0("(", toString(x), ")"))

# filter the data and select column to merge
df_filtered <- select(df_filtered, -SQ02S03_allsteps_list)
df_filtered_merge = df_filtered[c("idstud","RowNumber","SQ02S03_allsteps_timestamp")]
...
result_all_unit_time_Q3=result_all_unit_time_Q3%>%
  group_by(idstud) %>%
  fill(SQ02S03_allsteps_timestamp, .direction = "downup")
result_all_unit_time_Q3
```

### Total time taken
Create a new column SQ02S03_total_time. Group by idstud, SQ02S03_total_time = sum the value of the column SQ02S03_time for each idstud.

```{r}
# Group by idstud and calculate the sum of SQ02S03_time
result <- result_time_complete_Q3 %>%
  group_by(idstud) %>%
  summarise(SQ02S03_total_time = sum(SQ02S03_time, na.rm = TRUE))
...
# result_total_time_Q3[c("idstud","SQ02S03_time","SQ02S03_total_time")]
```
...
```{r}
resultS02 <- result_total_time_Q3 %>%
  select(idstud, idstud_ori, ImportLogID, BlockName, BookletPart,BlockInPart, module_path, SQ02S03_click_sequence, SQ02S03_choice_sequence, SQ02S03_wordcount_punc, SQ02S03_wordcount_no_punc, SQ02S03_raw_response, SQ02S03_allsteps_seq, SQ02S03_allsteps_seq_time, SQ02S03_allsteps_timestamp, SQ02S03_total_time)
# resultS02
```
...
Save the dataframe
```{r}
# write_sav(resultS02_save, "../../../new_data/grade5/Farm/SQ02S03_PSI.sav")

# write.csv(resultS02_save, file = 'csv_SQ02S03_PSI.csv', row.names = FALSE)
```
