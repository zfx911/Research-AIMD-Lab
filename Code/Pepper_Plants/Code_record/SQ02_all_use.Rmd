...
```{r}
result_test=head(result_all_use, n = 10)
result_test=result_test[, c('idstud', 'timeunixsec', 'timemilisec')]
result_test
```

```{r}
lag(result_test$timemilisec == 61)
```
...
```{r}
result_time_complete_all%>%
  filter(idstud == 51570105)%>%
  select('idstud', 'SQ02_time', 'timeunixsec', 'timemilisec')
```
...
Save the dataframe
```{r}
# write_sav(resultAll_save, "../../../new_data/grade9/Pepper_Plants/SQ02all_PSI.sav")
